...
class DatabaseHandler:
    ...
    def __init__(self, config):
        if not hasattr(self, 'initialized'):  # Prevent re-initialization
            ...
            mongo_user = (config.get("MONGO_USER") or "").strip()
            mongo_password = config.get("MONGO_PASSWORD") or ""
            mongo_host = (config.get("MONGO_URI") or "").strip()
            mongo_auth_db = (config.get("MONGO_AUTH_DB") or "").strip()

            if not mongo_host:
                raise ValueError("MONGO_URI is empty; expected host:port")

            # Build connection string (support unauthenticated local MongoDB)
            if mongo_user or mongo_password or mongo_auth_db:
                encoded_password = urllib.parse.quote_plus(mongo_password)
                auth_db_segment = mongo_auth_db
                MONGO_URI = f"mongodb://{mongo_user}:{encoded_password}@{mongo_host}/{auth_db_segment}"
            else:
            ...
            if mongo_auth_db:
                client_kwargs["authSource"] = mongo_auth_db  # authentication database
            ...
            if mongo_password:
                masked_uri = masked_uri.replace(urllib.parse.quote_plus(mongo_password), "****")
