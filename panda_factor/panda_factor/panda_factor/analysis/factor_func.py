...
# Legacy file readers formerly pointed at one contributor's workstation.
# These roots change only I/O location; all factor formulas below are unchanged.
LEGACY_DATA_ROOT = Path(
    os.environ.get("PANDA_FACTOR_DATA_ROOT", "data/legacy_factor")
).expanduser()
LEGACY_FACTOR_LIBRARY_ROOT = Path(
    os.environ.get("PANDA_FACTOR_LIBRARY_ROOT", str(LEGACY_DATA_ROOT / "factor_lib"))
).expanduser()
...
def read_kdata(start_date: str, end_date: str, field: list = []) -> pd.DataFrame:
    ...
    df_list = []
    folder_path = LEGACY_DATA_ROOT / "data_daily_kline"
    for path in os.listdir(folder_path):
        date = path[:-4]
        if start_date <= date <= end_date:
            df_temp = pd.read_csv(folder_path / path)
...
def read_kdata_min(start_time: str, end_time: str, code_list: list = []) -> pd.DataFrame:
    ...
    df_list = []
    folder_path = LEGACY_DATA_ROOT / "data_1min_kline"
    for code in code_list:
        path = folder_path / f'{code}.csv'
...
def read_capital_flow(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    df_list = []
    folder_path = LEGACY_DATA_ROOT / "data_capital_flow"
    for path in os.listdir(folder_path):
        date = path[:-4]
        if start_date <= date <= end_date:
            df_temp = pd.read_csv(folder_path / path)
...
def read_north(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    df_list = []
    folder_path = LEGACY_DATA_ROOT / "data_north"
    for path in os.listdir(folder_path):
        date = path[:-4]
        if start_date <= date <= end_date:
            df_temp = pd.read_csv(folder_path / path)
...
def read_margin_trading(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    df_list = []
    folder_path = LEGACY_DATA_ROOT / "data_margin_trading"
    for path in os.listdir(folder_path):
        date = path[:-4]
        if start_date <= date <= end_date:
            df_temp = pd.read_csv(folder_path / path)
...
def read_daily_basic(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    df_list = []
    folder_path = LEGACY_DATA_ROOT / "data_daily_basic"
    for path in os.listdir(folder_path):
        date = path[:-4]
        if start_date <= date <= end_date:
            df_temp = pd.read_csv(folder_path / path)
...
def read_factor(factor_name_list: list, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    factor_path_base = LEGACY_FACTOR_LIBRARY_ROOT
    ...
    for factor_name in factor_name_list:
        ...
        for path in os.listdir(factor_path):
            date = path[:-4]
            if start_date <= date <= end_date:
                df_temp = pd.read_csv(factor_path / path)
                df_temp['trade_date'] = date
                df_list.append(df_temp)
...
def read_barra(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    :param end_date: End date of data (inclusive)
    """
    path_barra = LEGACY_DATA_ROOT / "data_barra"
    df_list = []
    for path in os.listdir(path_barra):
        date = path[:-4]
        if start_date <= date <= end_date:
            df_list.append(pd.read_csv(path_barra / path))
