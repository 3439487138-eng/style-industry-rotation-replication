...
| Teacher public-index project (`data/open_source/*.csv`, method statement only) | `data/public_index_snapshot/*.csv`, `tools/prepare_open_index_data.py`, `src/strategy/{data,factors}.py` | Six real public-index snapshots; momentum, reversal, low-volatility and drawdown public proxy | Complete medium-fidelity adaptation | Yes, default | Public snapshots are versioned with SHA-256 verification; exact recovered-engine windows/weights were unavailable and remain explicit parameters |
| Existing neutralization, factor and performance fragments | `src/strategy/*.py` | Optional stock-panel style exposures, OLS neutralized score, periodic signals, equal-weight execution, costs, NAV, metrics and artifacts | Complete code path; data unavailable | Optional | Requires real market-cap, turnover and point-in-time industry fields; none are fabricated |
| `run_replication.py` | same path | Validate production inputs, load `strategy.adapter:run_strategy`, execute the full chain and generate auditable reports | Complete orchestration | Yes | Missing selected-profile inputs fail nonzero and list required files |
