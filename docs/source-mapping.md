...
| Teacher public-index project (`data/open_source/*.csv`, method statement only) | `tools/prepare_open_index_data.py`, `src/strategy/{data,factors}.py` | Six real index snapshots; momentum, reversal, low-volatility and drawdown public proxy | Complete medium-fidelity adaptation | Yes, default | Raw files remain outside this repository; exact recovered-engine windows/weights were unavailable and are explicit parameters |
